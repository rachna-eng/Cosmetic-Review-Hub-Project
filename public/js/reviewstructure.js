...
(function ($) {
  ...
  Rating.prototype = {
    ...
    initSlider: function (options) {
      ...
      self.min =
        typeof options.min !== "undefined"
          ? options.min
          : self._parseAttr("min", options);
      self.max =
        typeof options.max !== "undefined"
          ? options.max
          : self._parseAttr("max", options);
      self.step =
        typeof options.step !== "undefined"
          ? options.step
          : self._parseAttr("step", options);
    ...
    init: function (options) {
      ...
      self.clearButtonBaseClass = !isEmpty(options.clearButtonBaseClass)
        ? options.clearButtonBaseClass
        : "clear-rating";
      self.clearButtonActiveClass = !isEmpty(options.clearButtonActiveClass)
        ? options.clearButtonActiveClass
        : "clear-rating-active";
      ...
      if (
        typeof self.$rating == "undefined" &&
        typeof self.$container == "undefined"
      ) {
    ...
    generateRating: function () {
      ...
      if (self.inactive) {
        ...
        self.$container
          .removeClass("rating-disabled")
          .addClass("rating-active");
      }

      if (
        typeof self.$caption == "undefined" &&
        typeof self.$clear == "undefined"
      ) {
        if (self.rtl) {
      ...
      if (!isEmpty(self.containerClass)) {
        self.$container
          .removeClass(self.containerClass)
    ...
    fetchCaption: function (rating) {
      ...
      if (typeof self.starCaptionClasses == "function") {
        css = isEmpty(self.starCaptionClasses(val))
          ? self.clearCaptionClass
          : self.starCaptionClasses(val);
      } else {
        css = isEmpty(self.starCaptionClasses[val])
          ? self.clearCaptionClass
          : self.starCaptionClasses[val];
      ...
      if (typeof self.starCaptions == "function") {
        var cap = isEmpty(self.starCaptions(val))
          ? self.defaultCaption.replace(/\{rating\}/g, val)
          : self.starCaptions(val);
      } else {
        var cap = isEmpty(self.starCaptions[val])
          ? self.defaultCaption.replace(/\{rating\}/g, val)
          : self.starCaptions[val];
    ...
    getValueFromPosition: function (pos) {
      ...
      if (self.rtl) {
        val =
          self.min +
          Math.floor((self.diff * percentage) / self.step) * self.step;
      } else {
        val =
          self.min +
          Math.ceil((self.diff * percentage) / self.step) * self.step;
      }
      if (val < self.min) {
        val = self.min;
